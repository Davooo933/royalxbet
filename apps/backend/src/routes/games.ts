...
function bytesToFloat01(bytes: Buffer, offset = 0): number {
...
router.post('/bet', async (req: any, res) => {
  ...
  const { gameKey, wagerCents, selection, clientSeed } = parsed.data as { gameKey: typeof ALL_GAME_KEYS[number]; wagerCents: number; selection: any; clientSeed: string };
  ...
  const randInt = (min: number, max: number, bytes: Buffer, off = 0) => {
  ...
  const slotLike = (
    ...
    if (best) {
      const [sym, cnt] = best;
      if (cnt >= 3) mult = (sym.endsWith('W') || sym === 'W' ? 3 : 2) * (cnt - 2);
    }
  ...
  switch (gameKey) {
    ...
    case 'BLACKJACK': {
      ...
      const draw = (i: number) => shoe[i % shoe.length];
      const cardValue = (c: number) => {
        const r = (c % 13) + 1; // 1..13
        if (r >= 10) return 10;
        if (r === 1) return 11; // ace as 11 initially
        return r;
      };
      ...
      let idx = 4;
      while (handValue(player) < 12 + risk) { player.push(draw(bytes[idx++])); if (player.length > 8) break; }
      // dealer hits to 17
      while (handValue(dealer) < 17) { dealer.push(draw(bytes[idx++])); if (dealer.length > 8) break; }
      ...
      else if (dv > 21) { outcome = 'WIN'; mult = 2.0; }
      else if (isBJ && !dealerBJ) { outcome = 'WIN'; mult = 2.5; }
      else if (pv > dv) { outcome = 'WIN'; mult = 2.0; }
