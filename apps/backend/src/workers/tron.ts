...
async function getCurrentBlock(): Promise<number> {
  ...
  return now.block_header.raw_data.number as number;
}

async function creditDepositIfNew(txHash: string, toAddress: string, amountUSDT: number) {
  ...
  try {
    ...
    setInterval(async () => {
      try {
        ...
        if (toScanUntil <= lastBlock) return;
        ...
        for (const ev of events) {
          ...
          const amountUSDT = val / 1_000_000;
  ...
  setInterval(async () => {
    try {
      ...
      for (const tx of pendings) {
        ...
        const amount = Math.abs(tx.amountCents) / 100 * 1_000_000; // to 6 decimals
        try {
